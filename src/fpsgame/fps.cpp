...
namespace game
{
    ...
    void spawnplayer(fpsent *d)   // place at random spawn
    {
        ...
        if(d==player1)
        {
	        SbPy::triggerEventInt("local_spawn", d->clientnum);
    ...
    void killed(fpsent *d, fpsent *actor)
    {
        ...
        else if(d==actor || actor->type==ENT_INANIMATE)
        {
            ...
            SbPy::triggerEventInt("suicide", d->clientnum);
        }
        else if(isteam(d->team, actor->team))
        {
            ...
            SbPy::triggerEventIntInt("teamkill", actor->clientnum, d->clientnum);
        }
        else
        {
            if(d==player1) conoutf(contype, "\f2you got fragged by %s", aname);
            else conoutf(contype, "\f2%s fragged %s", aname, dname);
            SbPy::triggerEventIntInt("frag", actor->clientnum, d->clientnum);
        }
        deathstate(d);
		ai::killed(d, actor);
    }
    ...
    void timeupdate(int secs)
    {
        ...
        {
            SbPy::triggerEvent("Intermission", 0);
            intermission = true;
            player1->attacking = false;
    ...
    void clientdisconnected(int cn, bool notify)
    {
        ...
        SbPy::triggerEventInt("disconnect", cn);
        removeweapons(d);
    ...
    void startmap(const char *name)   // called just after a map load
    {
        ...
        SbPy::triggerEventStrInt("map_change", name ? name : "", gamemode);
