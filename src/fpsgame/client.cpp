...
namespace game
{
    ...
    void gamedisconnect(bool cleanup)
    {
        ...
        SbPy::triggerEventInt("disconnect", player1->clientnum);
        player1->clientnum = -1;
        sessionid = 0;
        mastermode = MM_OPEN;
        messages.setsize(0);
    ...
    void parsemessages(int cn, fpsent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            case N_SERVINFO:                   // welcome messsage from the server
            {
                ...
                SbPy::triggerEventInt("connect", player1->clientnum);
                
                const ENetAddress *address = connectedpeer();
                string hostname;
                result(address && enet_address_get_host_ip(address, hostname, sizeof(hostname)) >= 0 ? hostname : "");
                SbPy::triggerEventIntStringString("local_connect_post", player1->clientnum, newstring(address && enet_address_get_host_ip(address, hostname, sizeof(hostname)) >= 0 ? hostname : ""), newstring(address ? address->port : -1));
            ...
            case N_WELCOME:
            {
                ...
                if(!hasmap) 
                {
	                initmap = true; // we are the first client on this server, set map
                	SbPy::triggerEvent("firstclient", 0);
                }
                break;
            }
            ...
            case N_PAUSEGAME:
            {
                ...
                SbPy::triggerEventInt("pause", gamepaused);
                break;
            }
            ...
            case N_TEXT:
            {
                ...
                SbPy::triggerEventIntString("chat", d->clientnum, text);
                break;
            }
            ...
            case N_SAYTEAM:
            {
                ...
                SbPy::triggerEventIntString("teamchat", t->clientnum, text);
                break;
            }
            ...
            case N_INITCLIENT:            // another client either connected or changed name/team
            {
                ...
                SbPy::triggerEventInt("connect", cn);
                break;
            }
            ...
            case N_SPAWN:
            {
                if(d)
                {
                    ...
                    SbPy::triggerEventInt("spawn", d->clientnum);
                    d->respawn();
                }
                parsestate(d, p);
                if(!d) break;
            ...
            case N_GUNSELECT:
            {
                ...
                SbPy::triggerEventIntInt("change_weapon", d->clientnum, d->gunselect);
                break;
            }
            ...
            case N_TAUNT:
            {
                ...
                SbPy::triggerEventInt("taunt", d->clientnum);
                break;
            }
            ...
            {
                int demos = getint(p);
            	PyObject *pTuple = PyTuple_New(demos);
				PyObject *pStr;
                if(!demos) conoutf("no demos available");
                else loopi(demos)
                {
                    getstring(text, p);
					pStr = PyString_FromString(text);
					PyTuple_SetItem(pTuple, i, pStr);
                    conoutf("%d. %s", i+1, text);
                }
                SbPy::triggerEventPyTuple("demolist", pTuple);
                break;
            }
            ...
            case N_MASTERMODE:
            {
                mastermode = getint(p);
                conoutf("mastermode is %s (%d)", server::mastermodename(mastermode), mastermode);
                SbPy::triggerEventInt("mastermode", mastermode);
                break;
            }
            ...
            case N_EDITMODE:
            {
                ...
                break;
                SbPy::triggerEventIntBool("editing", d->clientnum, d->state==CS_EDITING);
            }
            ...
            case N_SPECTATOR:
            {
                ...
                SbPy::triggerEventIntBool("spectating", s->clientnum, s->state==CS_SPECTATOR);
                break;
            }
            ...
            case N_SETTEAM:
            {
                ...
                SbPy::triggerEventIntIntString("changedteam", w->clientnum, reason, w->team);
                break;
            }
            ...
            case N_NEWMAP:
            {
                ...
                if(d && d!=player1)
                {
                	...
                	SbPy::triggerEventIntInt("newmap", d->clientnum, newsize);
                }
                break;
            }
