...
{   
    ...
    if(servername)
    {
        ...
        SbPy::triggerEventIntString("server_connect", serverport, servername);
...
void reconnect(const char *serverpassword)
{
    if(!connectname[0] || connectport <= 0)
    {
        ...
        SbPy::triggerEventInt("reconnect_fail", 0);
        return;
    }

    connectserv(connectname, connectport, serverpassword);
    SbPy::triggerEventInt("reconnect_success", 0);
}
...
void disconnect(bool async, bool cleanup)
{
    if(curpeer) 
    {
        ...
        SbPy::triggerEventInt("connection_disconnected", 0);
...
void trydisconnect()
{
    ...
    {
        conoutf("aborting connection attempt");
        SbPy::triggerEventInt("connection_aborting", 0);
        abortconnect();
    }
    else if(curpeer)
    {
        conoutf("attempting to disconnect...");
        SbPy::triggerEventInt("connection_killing", 0);
...
void gets2c()           // get updates from the server
{
    ...
    if(connpeer && totalmillis/3000 > connmillis/3000)
    {
        ...
        connmillis = totalmillis;
        ++connattempts;
        SbPy::triggerEventInt("connection_attempt", connattempts);
        if(connattempts > 3)
        {
            conoutf("\f3could not connect to server");
            SbPy::triggerEventInt("connection_failed", 0);
            abortconnect();
            return;
        }
    }
    ...
    switch(event.type)
    {
        case ENET_EVENT_TYPE_CONNECT:
            ...
            SbPy::triggerEventInt("connection_succeeded", 0);
            throttle();
